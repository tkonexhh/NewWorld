...
namespace MightyTerrainMesh
{
    ...
    {
        public static string s_DataPath = string.Format("{0}/Res/Terrain", Application.dataPath);
        ...
        {
            string path = string.Format("{0}/{1}/{1}_{2}.bytes", s_DataPath, dataName, meshId);
            FileStream stream = File.Open(path, FileMode.Open);
            _vec2Cache.Clear();
            _intCache.Clear();
            //lods 
            ...
            {
                Debug.LogError("meshes length does not match lods");
                return;
            }
