Shader "Toon/Lit Swaying Interactive"
{
    ...
    SubShader
    {
        ...
        uniform float3 _ExplosionPositions[100];//爆炸坐标
        uniform float _ExplosionRange[100];//爆炸范围
        uniform float _ExplosionPositionArray;
        ...
        void vert(inout appdata_full v)//
        {
            ...
            for (int i = 0; i < _PositionArray; i ++)
            {
                ...
                if (length(sphereDisp) < _Radius)
                {
                    sphereDisp *= radius; // position multiplied by radius for falloff
                    v.vertex.xz += clamp(sphereDisp.xz * step(_YOffset, v.vertex.y) * _Strength, -_MaxWidth, _MaxWidth);// vertex movement based on falloff and clamped
                    
                    //如果离人物靠的很近的话 ，让他不那么移动
                    v.vertex.x -= (step(0, v.vertex.y - _YOffset) * x / 2 * _SwayMax);// apply the movement if the vertex's y above the YOffset
                    v.vertex.z -= (step(0, v.vertex.y - _YOffset) * z / 2 * _SwayMax);
                }
            }

            // for (int i = 0; i < _ExplosionPositionArray; i ++)
            // {
                //     float3 dis = distance(_ExplosionPositions[i], wpos); // distance for radius
                //     float3 radius = 1 - saturate(dis / _ExplosionRange[i]); // in world radius based on objects interaction radius
                //     float3 sphereDisp = wpos - _ExplosionPositions[i]; // position comparison
                //     //sphereDisp *= _ExplosionRange[i];
                //     float2 newxy =v.vertex.xy clamp(sphereDisp.xz * step(_YOffset, v.vertex.y), -_MaxWidth, _MaxWidth);
                //     v.vertex.xy = smoothstep(newxy, v.vertex.xy, 0.5);
                // }
            }
